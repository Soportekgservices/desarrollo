                ...
                ${data.map(s => `
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn-action" style="background:#0284c7; color:white;" onclick='viewAcademicDistribution(${s.id}, ${JSON.stringify(s.nombre)})' title="Estructura Académica"><i class="fa-solid fa-folder-tree"></i></button>
                ...
                ${schools.map(c => `
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn-action" style="background:#0284c7; color:white;" onclick='viewAcademicDistribution(${c.id}, ${JSON.stringify(c.nombre)})' title="Estructura Académica"><i class="fa-solid fa-folder-tree"></i></button>
    ...
    document.getElementById('dynamicBoard').innerHTML = `
        <div class="card span-6">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                ...
                <div class="input-box">
                    <label>Hora de inicio <span style="color:var(--secondary); font-size:0.8rem;">(opcional — franja horaria)</span></label>
                    <input type="time" id="linkHoraIni" placeholder="Ej: 08:00">
                </div>
                <div class="input-box">
                    <label>Hora de fin <span style="color:var(--secondary); font-size:0.8rem;">(opcional)</span></label>
                    <input type="time" id="linkHoraFin" placeholder="Ej: 10:00">
                </div>
            </div>
...
async function submitLinkTestRequest() {
    ...
    const hora_inicio = document.getElementById('linkHoraIni')?.value || null;
    const hora_fin = document.getElementById('linkHoraFin')?.value || null;

    if (!id_prueba || !id_grado || !id_grupo || !fecha_inicio || !fecha_fin) return alert('Complete todos los campos obligatorios.');
    if (fecha_fin < fecha_inicio) return alert('La fecha final debe ser mayor o igual a la inicial.');

    // Validar coherencia de horas si se especifican
    if ((hora_inicio && !hora_fin) || (!hora_inicio && hora_fin)) {
        return alert('Si defines una franja horaria, debes especificar tanto la hora de inicio como la hora de fin.');
    }
    // Solo validar que hora_fin > hora_inicio si es el mismo día
    if (hora_inicio && hora_fin && fecha_inicio === fecha_fin && hora_fin <= hora_inicio) {
        return alert('En el mismo día, la hora de fin debe ser posterior a la hora de inicio.');
    }
    ...
    const { data: res, error } = await _s.rpc('crear_solicitud_aplicacion_segura', {
        ...
        p_hora_inicio: hora_inicio || null,
        p_hora_fin: hora_fin || null
    });

    if (error || res?.status === 'error') {
        console.error(error);
        return alert('Error al guardar: ' + (error?.message || res?.message));
    }
    alert('Solicitud enviada. Un administrador la revisará.');
    backToSchoolList();
}

async function viewAcademicDistribution(schoolId, schoolName) {
    document.getElementById('pageTitle').innerText = "Distribución Académica";
    document.getElementById('pageDesc').innerText = schoolName;

    const board = document.getElementById('dynamicBoard');
    board.innerHTML = `
        <div class="card span-6" style="padding: 30px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 25px;">
                <h3 style="margin:0;"><i class="fa-solid fa-folder-tree" style="color:var(--primary); margin-right:8px;"></i> Estructura y Distribución Académica</h3>
                <button class="btn-main" style="width:auto; padding:10px 25px; background:var(--secondary);" onclick="${sess.rol === 'admin' ? 'viewGlobalSchools()' : 'viewDist()'}">
                    <i class="fa-solid fa-arrow-left"></i> Volver
                </button>
            </div>
            <div id="distributionContainer" style="padding: 20px 0; display:flex; justify-content:center; align-items:center;">
                <div style="color:var(--secondary); font-size:1.1rem;"><i class="fa-solid fa-spinner fa-spin"></i> Cargando estructura...</div>
            </div>
        </div>
    `;

    const { data: res, error } = await _s.rpc('obtener_estructura_academica_colegio', {
        p_colegio_id: parseInt(schoolId)
    });

    const container = document.getElementById('distributionContainer');
    if (error || res?.status === 'error') {
        container.innerHTML = `<div style="color:var(--danger); font-weight:bold; padding:20px; text-align:center;"><i class="fa-solid fa-triangle-exclamation"></i> Error al cargar: ${error?.message || res?.message || 'Desconocido'}</div>`;
        return;
    }

    const { total_grados, total_grupos, total_estudiantes, desglose } = res;

    if (!desglose || desglose.length === 0) {
        container.innerHTML = `
            <div style="text-align:center; padding: 40px; color: var(--secondary); width:100%;">
                <i class="fa-solid fa-folder-open" style="font-size: 3rem; margin-bottom:15px; opacity:0.5;"></i>
                <p style="margin:0; font-size:1.1rem; font-weight:600;">Este colegio no tiene grados ni grupos configurados.</p>
                <p style="margin:5px 0 0 0; font-size:0.9rem;">Usa la opción de "Grados y Grupos" para configurarlos.</p>
            </div>
        `;
        return;
    }
    ...
    let html = `
        <div style="width: 100%;">
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 30px;">
                <div style="background: #f8fafc; border: 1.5px solid #e2e8f0; border-radius: 12px; padding: 15px; text-align: center;">
                    <div style="font-size: 1.5rem; font-weight: 800; color: var(--primary);">${total_grados}</div>
                    <div style="font-size: 0.85rem; color: var(--secondary); font-weight: 600; margin-top:2px;">Grados Habilitados</div>
                </div>
                <div style="background: #f8fafc; border: 1.5px solid #e2e8f0; border-radius: 12px; padding: 15px; text-align: center;">
                    <div style="font-size: 1.5rem; font-weight: 800; color: var(--primary);">${total_grupos}</div>
                    <div style="font-size: 0.85rem; color: var(--secondary); font-weight: 600; margin-top:2px;">Grupos Creados</div>
                </div>
                <div style="background: #f8fafc; border: 1.5px solid #e2e8f0; border-radius: 12px; padding: 15px; text-align: center;">
                    <div style="font-size: 1.5rem; font-weight: 800; color: #0f766e;">${total_estudiantes.toLocaleString()}</div>
                    <div style="font-size: 0.85rem; color: var(--secondary); font-weight: 600; margin-top:2px;">Total Estudiantes</div>
                </div>
            </div>
            ...
            <table class="data-table" style="width:100%; border-collapse: collapse; margin-top: 10px;">
                <thead>
                    <tr style="background: #f1f5f9; text-align:left; border-bottom: 2px solid #cbd5e1;">
                        <th style="padding: 12px 15px; font-weight:700;">Grado</th>
                        <th style="padding: 12px 15px; font-weight:700; text-align: center; width: 200px;">Estudiantes en Grado</th>
                        <th style="padding: 12px 15px; font-weight:700;">Grupos y su Distribución (Estudiantes por Grupo)</th>
                    </tr>
                </thead>
                <tbody>
                    ${desglose.map(g => {
                        const gruposBreakdown = g.grupos && g.grupos.length > 0
                            ? g.grupos.map(gr => `
                                <div style="display: inline-flex; align-items: center; background: #e0f2fe; color: #0369a1; padding: 6px 12px; border-radius: 20px; font-size: 0.9rem; font-weight: bold; margin: 3px 6px 3px 0; border: 1px solid #bae6fd;">
                                    <span style="opacity: 0.75; margin-right: 4px;">Grupo</span> ${esc(gr.grupo_nombre)}: 
                                    <span style="background: #0369a1; color: white; border-radius: 50%; width: 22px; height: 22px; display: inline-flex; justify-content: center; align-items: center; font-size: 0.75rem; margin-left: 6px; font-weight: 800;">${gr.total_estudiantes_grupo}</span>
                                </div>
                            `).join('')
                            : `<span style="color:var(--secondary); font-size:0.9rem; font-style:italic;">Sin grupos creados</span>`;

                        return `
                            <tr style="border-bottom: 1px solid #e2e8f0;">
                                <td style="padding: 15px; font-size: 1.05rem; font-weight: 700; color: var(--primary);">
                                    <i class="fa-solid fa-graduation-cap" style="color:var(--primary); margin-right:6px; opacity:0.7;"></i> Grado ${esc(g.grado_nombre)}
                                </td>
                                <td style="padding: 15px; text-align: center;">
                                    <span class="badge" style="background: #f1f5f9; color: #475569; font-size: 0.95rem; font-weight: 800; padding: 6px 14px; border-radius: 8px; border: 1px solid #e2e8f0;">
                                        ${g.total_estudiantes_grado} alumnos
                                    </span>
                                </td>
                                <td style="padding: 15px; vertical-align: middle; line-height: 1.6;">
